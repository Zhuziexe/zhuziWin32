...
namespace zhuzi {
    ...
    zhuziImageList::~zhuziImageList() { destroy(); }
    ...
    int zhuziImageList::add(const zhuziImage& image) {
        HBITMAP hBitmap = image.getHandle();
        if (!hBitmap) return -1;
    ...
    int zhuziImageList::addFromFile(const wchar_t* filePath) {
        zhuziImage img;
        if (!img.loadFromFile(filePath)) return -1;
        HBITMAP hBitmap = img.toHBITMAP();
        if (!hBitmap) return -1;
        int idx = add(hBitmap, nullptr);
    ...
    int zhuziImageList::addFromResource(int resourceId, const wchar_t* resourceType) {
        ...
        zhuziImage img;
        if (!img.loadFromResource(resourceId, resourceType)) return -1;
        HBITMAP hBitmap = img.toHBITMAP();
        if (!hBitmap) return -1;
        int idx = add(hBitmap, nullptr);
        DeleteObject(hBitmap);
    ...
    int zhuziImageList::addFromMemory(const void* data, size_t size) {
        zhuziImage img;
        if (!img.loadFromMemory(data, size)) return -1;
        HBITMAP hBitmap = img.toHBITMAP();
        if (!hBitmap) return -1;
        int idx = add(hBitmap, nullptr);
        DeleteObject(hBitmap);
    ...
    int zhuziImageList::addFromGdiplusImage(Gdiplus::Image* image) {
        ...
        if (!hBitmap) return -1;
        int idx = this->add(hBitmap, nullptr);
        DeleteObject(hBitmap);
        return idx;
    }
